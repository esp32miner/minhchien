...
def like_tweet(tweet_id, cookie_str):
    ...
    try:
        ...
        if response.status_code == 200 and '"favorited":true' in response.text:
            print(f"\033[1;32m✅ Like thành công tweet_id={tweet_id}")
            return True
        else:
            print(f"\033[1;31m❌ Like lỗi, kiểm tra lại cookie")
    except Exception as e:
        print(f"\033[1;31m❌ Lỗi Like")
    return False
...
def follow_twitter(user_id, cookie_str):
    ...
    try:
        ...
        if response.status_code == 200 and '"following":true' in response.text:
            print(f"\033[1;32m✅ Follow thành công user_id={user_id}")
            return True
        else:
            print(f"\033[1;31m❌ Follow lỗi, kiểm tra lại cookie")
    except Exception as e:
        print(f"\033[1;31m❌ Lỗi Follow")
    return False
...
if os.path.exists(cookie_file):
    ...
    if twitter_cookie:
        new_cookie = input(f"\033[1;32m🚀 Nhấn Enter dùng Cookie cũ {Fore.BLUE}or \033[1;32mnhập cookie mới {Fore.RED}(tại đây)\033[1;32m: \033[1;33m").strip()
...
while True:
    ...
    if ads_id in ads_id_da_lam:
       lap_lai_ads += 1
       print(f"\033[1;33m⚠️ Bỏ qua job trùng lặp: ads_id={ads_id} ({lap_lai_ads} lần)")
    ...
    if object_id in object_id_da_lam:
      print(f"\033[1;33m⚠️ Đã follow/like object_id={object_id} trước đó. Bỏ qua.")
      baoloi(ads_id, object_id, account_id, nhanjob["data"]["type"])
      time.sleep(2)
      continue
